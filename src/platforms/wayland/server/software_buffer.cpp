...
mgw::SoftwareBuffer::SoftwareBuffer(
...
std::shared_ptr<mg::NativeBuffer> mgw::SoftwareBuffer::create_native_handle()
{
    auto buffer = std::make_shared<mgw::NativeBuffer>();
    *static_cast<MirBufferPackage*>(buffer.get())= *to_mir_buffer_package();
    return buffer;
}

std::shared_ptr<mg::NativeBuffer> mgw::SoftwareBuffer::native_buffer_handle() const
{

...
std::shared_ptr<mir::graphics::wayland::NativeBuffer> to_native_buffer(MirBufferPackage const& package)
{
    auto buffer = std::make_shared<mir::graphics::wayland::NativeBuffer>();
...
mclw::ClientBuffer::ClientBuffer(
...
mclw::ClientBuffer::~ClientBuffer() noexcept
{
...
std::shared_ptr<mcl::MemoryRegion> mclw::ClientBuffer::secure_for_cpu_write()
{
...
geom::Size mclw::ClientBuffer::size() const
{
    return rect.size;
}

geom::Stride mclw::ClientBuffer::stride() const
{
    return geom::Stride{creation_package->stride};
}

MirPixelFormat mclw::ClientBuffer::pixel_format() const
{
    return buffer_pf;
}

std::shared_ptr<mir::graphics::NativeBuffer> mclw::ClientBuffer::native_buffer_handle() const
{
    creation_package->age = age();
    return creation_package;
}

void mclw::ClientBuffer::update_from(MirBufferPackage const&)
{
}

void mclw::ClientBuffer::fill_update_msg(MirBufferPackage& package)
{
    package.data_items = 0;
    package.fd_items = 0;
}

MirBufferPackage* mclw::ClientBuffer::package() const
{
    return creation_package.get();
}

void mclw::ClientBuffer::egl_image_creation_parameters(EGLenum*, EGLClientBuffer*, EGLint**)
{

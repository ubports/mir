...
namespace
{
bool is_wayland_server(mcl::ClientContext* context)
{
    MirModuleProperties module_properties;

    context->populate_graphics_module(module_properties);

    return strncmp(module_properties.name, "mir:wayland", strlen("mir:wayland")) == 0;
}
}
    ...
    std::shared_ptr<mir::logging::Logger> const& /*logger*/)
{
    ...
    return mir::make_module_ptr<mclw::ClientPlatform>(context,
                                                      mcl::EGLNativeDisplayContainer::instance());
}
...
is_appropriate_module(mcl::ClientContext* context)
{
    ...
    return is_wayland_server(context);
}
